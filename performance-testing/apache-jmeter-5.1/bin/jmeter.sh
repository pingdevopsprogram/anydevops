...
HEAP="-Xms4g -Xmx4g"
NEW="-XX:NewSize=2g -XX:MaxNewSize=2g"
...
#Kerberos defines for IWA testing
SUBJECT_CREDS_ONLY="-Djavax.security.auth.useSubjectCredsOnly=false"
#KERB5_DEBUG="-Dsun.security.krb5.debug=true"
KERB5_CONF="-Djava.security.krb5.conf=krb5.conf"
LOGIN_CONF="-Djava.security.auth.login.config=login.conf"
#DEBUG="-Xdebug -Xrunjdwp:transport=dt_socket,address=8787,server=y,suspend=n"
#SSL_DEBUG="-Djavax.net.debug=sslctx,session,sessioncache,handshake"

JVM_ARGS="$JVM_ARGS $DUMP $HEAP $NEW $SURVIVOR $TENURING $RMIGC $PERM $DDRAW $SUBJECT_CREDS_ONLY $KERB5_CONF $LOGIN_CONF $DEBUG $SSL_DEBUG $KERB5_DEBUG"
